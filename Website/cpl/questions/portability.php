...
class Portability extends Question {	
	...
	*/
	...
	int arr[size];
		...
		$cpp4 = parent::cppSays("Since you can't ship each possible driver and library with your code, backwards compatibility is somehow related to portability. If your code is not backwards compatible, you have a large chance stuff will break at the client side. In C++ backwards compatibility is an important issue. More than 1300 pages were written in order to keep the language backwards compatbile".parent::bib()->cite(1).". I agree that's a huge cost to achieve compatibility. However C++ is used with a lot of legacy code what makes the effort justified. However a glue language like Lua probably doesn't need to be backwards compatible since the glue logic is used for only one application.");
