...
class Portability extends Question {	
	...
	*/
	...
	int arr[size];
		...
		$popoverMMX = $this->popover("MMX", "MMX", "MMX is an instruction set that provides single instruction multiple data. This way a programmer can perform multiple operations at the same time.");
		$popoverSSE = $this->popover("SSE", "SSE", "SSE can easily be described as a more advanced MMX.");

		$lua0 = parent::luaSays("One of the biggest tasks for designers of programming languages is making their language more portable. The designers of Lua however wanted a portable system from the start.");
		$cSharp0 = parent::cSharpSays("Can you give any evidence for this statement?");
		$lua1 = parent::luaSays("Well, Lua originates from two languages: DEL and SOL. The designers of Lua argued that since both languages were used by a <a href=\"evolution.php\">slightly different audience</a>, they could not make much assumptions about their platform configurations.");
		...
		$lua4 = parent::luaSays("The main issue is that libraries have to be rewritten to support a new version of Lua. One could of course solve this problem by running two Lua interpreters side by side. However this would make matters complicated and would probably result in bad performance. Furthermore since the interpreter is highly portable, it can't take much advantage on special characteristics of the platform it runs on.");
		$cpp1 = parent::cppSays("I agree, since performance is very important in C++, this would already be a strong argument against using a virtual machine, not to mention a platform-independent interpreter.");
		...
		$java2 = parent::javaSays("Well, if you compile your system to machine code, you have to make assumptions about the system. With a virtual machine, you postpone this decisions until you run the program. If the processor for instance supports " . $popoverMMX . " or " . $popoverSSE . ", the virtual machine can optimize the code in order to take advantage of the extended instruction set. The same holds for instance for GPU's with OpenCL support.");
		...
		$cpp4 = parent::cppSays("Since you can't ship each possible driver and library with your code, backwards compatibility is somehow related to portability. If your code is not backwards compatible, you have a large chance stuff will break at the client side. In C++ backwards compatibility is an important issue. The C++ contains 40 pages with only possible compatibility issues".parent::bib()->cite(1).". I agree that's a huge cost to achieve compatibility, but C++ is used with a lot of legacy code which makes the effort justified. However a glue language like Lua probably doesn't need to be backwards compatible since the glue logic is used for only one application.");
		$lua6 = parent::luaSays("True, another reason I think is that Lua remains an interpreted language. The more complex the run-time interpreter, the slower the program will run. Therefore legacy features imply a bloated language and thus bloated programs.");
