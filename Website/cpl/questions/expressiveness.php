...
class Expressiveness extends Question {	
	...
	*/
	...
	public function answers(){
		...
		$cSharp1 = parent::cSharpSays("C# has a similar feature: the fact that you are working with a coroutine doesn't has to be specified. One simply uses the yield statement in order to emit an element of the virtual list.");
		...
		$lua5_0 = parent::luaSays("Not in Lua. Coroutines in Lua have their own execution stack, root set,... Therefore one can look at these methods as an independent method who simply runs by need.");
		//TODO: overgang naar serialisatie
		...
		$cSharp3 = parent::cSharpSays("You are right in one way: in C# you have to specify how the object is serialized/deserialized. The reason is that a system can change and the fields of a class can be modified. I guess the Java will break when you modify the class definition? Since a C# programmer specifies how to deserialize data, he can implement a deserializer for different versions in the language.");
		...
		$lua6 = parent::luaSays("That's true. However Lua has no built-in functionalities to serialize objects. But since each object is a table, it's not that hard to build a generic method to do the job. Methods aren't a problem either: the virtual machine can send Lua code through the network. Since it's interpreted at the other side, their are no compatibility issues.");
		$cSharp4 = parent::cSharpSays("");
		...
		return $cSharp0.$lua0.$cpp0_0.$java0.$cpp0.$java1.$cpp1.$lua1.$java2_0.$lua2.$java3.$lua3.$cSharp1.$lua4.$cSharp2.$java2.$lua5.$java4.$lua5_0.$cpp3.$java5.$cSharp3.$java6.$cpp4.$lua6;
	}
