...
class Expressiveness extends Question {	
	...
	*/
	...
	public function answers(){
		...
		$lua3 = parent::luaSays("Well, as I already said a typical application is data generation or data modification. So take a look at the following code: ".$code2."You see a function that returns the chemical symbols of all noble gasses. No we can turn this method into a coroutine and use that method in a loop. As long as there are still symbols, the coroutine is still alive and we can print symbols. Now i guess one can imagine a method where we simply generate all prime numbers. One can write this function as a coroutine to generate for instance all prime numbers smaller than 42.");
		...
		$java6 = parent::javaSays("The structure of the class is also encoded (of course together with its superclasses).");
		...
		$lua6 = parent::luaSays("That's true. However Lua has no built-in functionalities to serialize objects. But since each object is a table, it's not that hard to build a generic method to do the job. Methods aren't a problem either: the virtual machine can send code through the network.");
