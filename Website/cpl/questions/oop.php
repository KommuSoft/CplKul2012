...
//Prototype-based programming is a style of object-oriented programming in which classes are not present, and behavior reuse (known as inheritance in class-based languages) is performed via a process of cloning existing objects that serve as prototypes. This model can also be known as classless, prototype-oriented or instance-based programming. Delegation is the language feature that supports prototype-based programming.
		...
		$lua0 = parent::luaSays("The authors of Lua didn't want to turn Lua into an object-oriented language, because they didn't want to fix a programming paradigm for it. However, the mechanism of classes and objects can be implemented with tables. A table is the main, and the only, data structure in Lua. Tables are implemented as associative arrays. That are arrays where not only numbers can be used as indexes, but any other type (except nil). The other 7 types are number, string, table, function, userdata, boolean and thread. In that way, the keys of the table can be used as the names of the fields and methods by using the type string. Above that, functions are first-class values. This means that functions can be assigned to global and local variables, passed as arguments to functions, returned from functions and stored in tables. So, functions can be stored as the values in a table.");
