...
class LanguageDesign extends Question {	
	...
	*/
	public function answers(){
		...
		$lua3 = parent::luaSays("The main reason is it's simplicity".parent::bib()->cite(0).". However there are other factors: first of all Lua was and still is a scripting language. That means there is no compiler step. Since the program is also lazily loaded, doing type checking would not discover that much issues.");
		$cSharp1 = parent::cSharpSays("Yes, but Lua has a compiler now, and since I recall Lua doesn't think that much about backward compatibility, Lua could abandon the dynamic typing idea. C# tries to navigate in the middle of dynamic and static typing: in the earlier versions of C# variables had to be stricly typed. Since 2010 however one can use the 'dynamic' keyword. By using dynamic, one can call methods where the compiler is not sure wether these methods exist. Perhaps Lua could become more static typed by allowing the programmer to annotate variables with types who are then checked by the compiler?");
		$haskell1 = parent::haskellSays("I propose we look for other aspects of language design, since we will also have another discussion about typing. An important decision in a programming language is it's execution machanism. I think Haskell is the only lazy language here. Why isn't laziness implemented in Lua, C++, ...?");
		...
		$lua6 = parent::luaSays("Lua wasn't intended to become a glue language. However LucasArts created a game and used Lua as a scripting engine. Therefore Lua got a lot of attention from the gaming industry. Probably it's generic and dynamic design were key features for it's succes.");
		$cpp3 = parent::cppSays("What do you mean?");
		$lua7 = parent::luaSays("Typically scripts for games are already written in an early stage of the development. However at that time some aspects are not decided yet. Since Lua is that generic, a developer changing his mind will not yield that much additional workt to addapt the scripts on new code in the host language.");
