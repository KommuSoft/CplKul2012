...
class LanguageDesign extends Question {	
	...
	*/
	...
	print(a+b)
		...
		$lua3 = parent::luaSays("The main reason is it's simplicity".parent::bib()->cite(0).". However there are other factors: first of all Lua was and still is a scripting language. This means there is no compiler step. Since the program is also lazily loaded, doing type checking would not discover that much issues.");
		$cSharp1 = parent::cSharpSays("Yes, but Lua has a compiler now, and since I recall Lua doesn't think that much about backward compatibility, Lua could abandon the dynamic typing idea. C# tries to navigate in the middle of dynamic and static typing: in the earlier versions of C# variables had to be strictly typed. Since C# 4.0 however one can use the 'dynamic' keyword".parent::bib()->cite(21).". By using dynamic, one can call methods where the compiler is not sure wether these methods exist. Perhaps Lua could become more statically typed by allowing the programmer to annotate variables with types who are then checked by the compiler?");
